...
function App() {
  ...
  const handleSubmit = async (e) => {
    ...
    setLoading(false)
    const newResponses = [{prompt: prompt, response: data.choices[0].text}, ...responses]
    ...
    <div className="App">
      <h1>OpenAI Text Prompt Tester</h1>
        <Form className="prompt-container" onSubmit={e => handleSubmit(e)}>
          ...
          <Form.Control type="text" value={prompt} onChange={(e) => setPrompt(e.target.value)} as="textarea"
          placeholder="Enter your prompt here! Feel free to leave the page and refresh, your prompts will still be there! :D" style={{"width": "40vw", "height": "10vw"}}/>
          </Form.Group>
        ...
        {responses.map((response) => {
          return (<PromptResponse prompt={response.prompt} response={response.response}/>)
        })}
        </div>
